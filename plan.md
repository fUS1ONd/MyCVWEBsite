...
- [x] Создать PostsListPage компонент (/blog), требует авторизации
- [x] Реализовать PostCard компонент (превью поста)
- [x] Добавить пагинацию для списка постов
- [x] Реализовать фильтры (по дате, поиск по заголовку)
- [x] Добавить skeleton loading для постов
- [ ] Оптимизировать изображения в превью (thumbnails)

### 7.2 Страница поста
- [x] Создать PostDetailPage компонент (/blog/{slug})
- [x] Реализовать рендеринг markdown контента (react-markdown + syntax highlighting)
- [x] Добавить поддержку встроенных изображений в контенте
- [x] Добавить поддержку встроенных видео (YouTube, Vimeo embeds)
- [x] Реализовать share кнопки (Twitter, Facebook, LinkedIn, VK)
- [x] Добавить кнопку "Назад к списку постов"
- [ ] Реализовать навигацию к предыдущему/следующему посту

### 7.3 Комментарии
- [x] Создать CommentsSection компонент
- [x] Реализовать Comment компонент (аватар, имя, дата, текст)
- [x] Добавить форму для создания комментария (textarea + submit)
- [x] Реализовать вложенные комментарии (reply to comment)
- [x] Добавить возможность редактирования своего комментария
- [x] Добавить возможность удаления своего комментария (с подтверждением)
- [x] Реализовать optimistic updates для комментариев
- [x] Добавить валидацию комментариев на frontend (max length, required)
- [x] Реализовать auto-scroll к новому комментарию после создания
